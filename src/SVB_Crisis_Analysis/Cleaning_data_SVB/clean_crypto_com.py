...
def clean_file(filepath, output_dir, long_gap_log):
    ...
    col_order = ["symbol", TIME_MS_COL, TIME_UTC_COL, "open", "high", "low",
                 "close", VOLUME_COL, "imputed", "gap_size", "impute_method"]
...
def main():
    parser = argparse.ArgumentParser(description="Crypto.com OHLCV Data Cleaner")
    parser.add_argument("--input",  required=True, help="Directory containing crypto_com_*.csv files")
    parser.add_argument("--output", required=True, help="Directory to write cleaned CSV files")
    args = parser.parse_args()

    os.makedirs(args.output, exist_ok=True)

    all_files = sorted(glob.glob(os.path.join(args.input, "crypto_com_*.csv")))

    if not all_files:
        print(f"\n  ERROR: No crypto_com_*.csv files found in:\n  {args.input}")
        return

    print(f"\n{'='*60}")
    print(f"  Crypto.com OHLCV Cleaner")
